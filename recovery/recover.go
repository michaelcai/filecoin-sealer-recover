 ...
 func RecoverSealedFile(ctx context.Context, rp export.RecoveryParams, parallel uint, sealingResult string, sealingTemp string, pieceCID string, piecePath string) error {
				 ...
				 for _, sector := range rp.SectorInfos {
								 ...
								 go func(sector *export.SectorInfo) {
												 ...
												 mkdirAll(sdir)
												 tempDir, err := homedir.Expand(fmt.Sprintf("%s/recover-%d", sdir, sector.SectorNumber))
												 if err != nil {
